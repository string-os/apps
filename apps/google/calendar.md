...
```act.agenda
CLI TOKEN=$(gcloud auth print-access-token 2>/dev/null); [ -z "$TOKEN" ] && echo "Need OAuth — see /open requirements.md" && exit 1; START=$(date -u -d "{start}" +%Y-%m-%dT%H:%M:%SZ); END=$(date -u -d "{end} 23:59:59" +%Y-%m-%dT%H:%M:%SZ); curl -s -G -H "Authorization: Bearer $TOKEN" --data-urlencode "timeMin=$START" --data-urlencode "timeMax=$END" --data-urlencode "singleEvents=true" --data-urlencode "orderBy=startTime" --data-urlencode "maxResults=250" "https://www.googleapis.com/calendar/v3/calendars/primary/events" | python3 -c "import json,sys; ev=json.load(sys.stdin).get('items',[]); print(f'## {len(ev)} events\n') if ev else print('No events in range.'); [print(f\"### {e.get('summary','(no title)')}\n- **When:** {e.get('start',{}).get('dateTime', e.get('start',{}).get('date',''))} → {e.get('end',{}).get('dateTime', e.get('end',{}).get('date',''))}\n- **Location:** {e.get('location','-')}\n- **ID:** {e.get('id','')}\n\") for e in ev]"
...
```act.create_event
CLI TOKEN=$(gcloud auth print-access-token 2>/dev/null); [ -z "$TOKEN" ] && echo "Need OAuth — see /open requirements.md" && exit 1; TZ_NAME=$(timedatectl show -p Timezone --value 2>/dev/null || echo "UTC"); START=$(date -d "{when}" +%Y-%m-%dT%H:%M:%S); END=$(date -d "{when} + {duration} minutes" +%Y-%m-%dT%H:%M:%S); curl -s -X POST -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -d "{\"summary\":\"{title}\",\"start\":{\"dateTime\":\"$START\",\"timeZone\":\"$TZ_NAME\"},\"end\":{\"dateTime\":\"$END\",\"timeZone\":\"$TZ_NAME\"}}" "https://www.googleapis.com/calendar/v3/calendars/primary/events" | python3 -c "import json,sys; r=json.load(sys.stdin); print(f'✓ Created: {r[\"summary\"]}\n- **When:** {r[\"start\"][\"dateTime\"]} → {r[\"end\"][\"dateTime\"]}\n- **ID:** {r[\"id\"]}\n- **Link:** {r.get(\"htmlLink\",\"-\")}') if 'id' in r else print(f'✗ Error: {r}')"
...
```act.allday
CLI TOKEN=$(gcloud auth print-access-token 2>/dev/null); [ -z "$TOKEN" ] && echo "Need OAuth — see /open requirements.md" && exit 1; START="{when}"; END=$(date -d "{when} + 1 day" +%Y-%m-%d); curl -s -X POST -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -d "{\"summary\":\"{title}\",\"start\":{\"date\":\"$START\"},\"end\":{\"date\":\"$END\"}}" "https://www.googleapis.com/calendar/v3/calendars/primary/events" | python3 -c "import json,sys; r=json.load(sys.stdin); print(f'✓ Created (all-day): {r[\"summary\"]} on {r[\"start\"][\"date\"]}\n- **ID:** {r[\"id\"]}') if 'id' in r else print(f'✗ Error: {r}')"
  title: string (required) "Event title"
  when: string (required) "Date (YYYY-MM-DD)"
```

```act.search_cal
CLI TOKEN=$(gcloud auth print-access-token 2>/dev/null); [ -z "$TOKEN" ] && echo "Need OAuth — see /open requirements.md" && exit 1; START=$(date -u -d "{start}" +%Y-%m-%dT%H:%M:%SZ); END=$(date -u -d "{end}" +%Y-%m-%dT%H:%M:%SZ); curl -s -G -H "Authorization: Bearer $TOKEN" --data-urlencode "q={query}" --data-urlencode "timeMin=$START" --data-urlencode "timeMax=$END" --data-urlencode "singleEvents=true" --data-urlencode "orderBy=startTime" "https://www.googleapis.com/calendar/v3/calendars/primary/events" | python3 -c "import json,sys; ev=json.load(sys.stdin).get('items',[]); print(f'## {len(ev)} match(es)\n') if ev else print('No matches.'); [print(f\"### {e.get('summary','(no title)')}\n- **When:** {e.get('start',{}).get('dateTime', e.get('start',{}).get('date',''))}\n- **ID:** {e.get('id','')}\n\") for e in ev]"
